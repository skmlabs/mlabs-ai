...
// Phase 5B (Vetic-demo): data source switched BACK to cached_reviews now that
// GMB v4.9 access has propagated. cached_reviews holds full history with reply
// state, so the KPI cards (avg TAT / % responded / need-reply / median TAT)
// and the Status filter become meaningful again — both restored in the inbox UI.
//
...
export async function GET(request: NextRequest) {
  ...
  // Date-range util gives "yesterday" as end (GMB metric lag); for reviews we
  // want end-of-day INCLUSIVE so today's replies show up.
  ...
  let locQuery = supabase
    .from("locations")
    .select("id, title, gmb_account_id, location_resource_name")
    .eq("user_id", user.id)
    .eq("is_active", true)
  ...
  // Locations dropdown — always returns ALL non-manual locations regardless
  // of the single-location filter, so the dropdown still lets the user switch.
  ...
  if (locs.length === 0) {
    return NextResponse.json({
      range: {
        key: rangeKey,
        label,
        start: customStart ?? toYmdUtc(start),
        end: customEnd ?? toYmdUtc(end),
      },
      reviews: [],
  ...
  const enrichedReviews = (reviews ?? []).map(r => ({
    ...
    google_review_name: r.google_review_name,
    ...
    first_response_tat_seconds: r.first_response_tat_seconds,
  }));
